 ...
 * DB IDs in URLs? primary key ist ok
 * dict zugriff?
"""
...
class A(object):
    ...
    def mapArtistToPersonAliasToRole(self, artistID, personAliasID, roleID):
        args = {'artist_id': artistID, 'person_alias_id': personAliasID, 'role_id': roleID}
        self.cur.execute("""insert into musicdb.artist__person_alias__role 
        (artist_id, person_alias_id, role_id) values (%(artist_id)s, %(person_alias_id)s, %(role_id)s)""", args)
...
class TestA(ut.TestCase):
    ...
    def test_mapArtistToPersonAliasToRole(self):
        cur = self.conn.cursor()
        artistID = a.insertArtist(cur, 'testArtist')
        personID = a.insertPersonAlias(cur, 'testPerson')
        roleID = a.getRoleID('Voice')
        a.mapArtistToPersonAliasToRole(cur, artistID, personID, roleID)
        cur.execute("""select a.name
        from musicdb.person_alias as p, musicdb.artist as a, musicdb.artist__person_alias__role as ap
...
if __name__ == '__main__':
    ...
    def personAlias__artist():
        ...
        a.mapArtistToPersonAliasToRole(tool, maynard, voice)
        a.mapArtistToPersonAliasToRole(circle, maynard, voice)
        a.mapArtistToPersonAliasToRole(circle, james, guitar)
        a.mapArtistToPersonAliasToRole(pumpkins, james, guitar)

        a.cur.execute("""
        select a.name, count(*) from 
        musicdb.person_alias as p, 
        musicdb.artist as a, 
        musicdb.artist__person_alias__role as apr
        where apr.artist_id = a.id and apr.person_alias_id = p.id
        group by a.name""")
        pprint.pprint( a.cur.fetchall() )
        
        a.cur.execute("""
        select p.name || ' spielt in ' || b.name as artistmembers, r.name from 
        musicdb.person_alias as p, 
        musicdb.artist as b, 
        musicdb.role as r,
        musicdb.artist__person_alias__role as apr
        where (b.id = apr.artist_id and p.id = apr.person_alias_id and apr.role_id = r.id) """)
        pprint.pprint( a.cur.fetchall())
    ...
    def personAlias_record_role():
        ...
        a.mapArtistToPersonAliasToRole(opeth, mikael, voice)
        a.mapArtistToPersonAliasToRole(opeth, mikael, guitar)
        a.mapArtistToPersonAliasToRole(tree, steven, voice)
        a.mapArtistToPersonAliasToRole(tree, steven, guitar)
    ...
    def fillDBWithTestData():
        print 'start inserting testdata'
        a = makeAwithConnAndCur()
        ...
        lazarus = a.insertSong('Lazarus')
        arriving = a.insertSong('Arriving Somewhere But Not Here')
        a.mapSongToVolume(lazarus, dead1)
        a.mapSongToVolume(arriving, dead1)
        ...
        a.mapPersonAliasToRecordToRole(steven, damn, voice)
        a.mapPersonAliasToRoleToSong(mikael, voice, lazarus)
        a.mapPersonAliasToRoleToSong(mikael, voice, arriving)
        a.mapPersonAliasToRoleToSong(mikael, guitar, arriving)
        #######################################
        sm = a.insertRecord('S&M')
        justice = a.insertRecord('...And Justice For All')
        sm2 = a.insertVolume(2, recordID=sm)
        ...
        sm1 = a.insertVolume(2, recordID=sm)
        mellon1 = a.insertVolume(1, 'dawn to dusk', mellon)
        mellon2 = a.insertVolume(2, 'twilight to starlight', mellon)
        tonight = a.insertSong('Tonight, Tonight')
        thirty = a.insertSong('Thirty-Three')
        a.mapSongToVolume(tonight, mellon1)
        a.mapSongToVolume(thirty, mellon2)
        ...
        just = a.insertSong('Just So You Know')
        billy = a.insertPersonAlias('Billy Howedel')
        a.mapArtistToPersonAliasToRole(circle, billy, guitar)
        #dummyPerson = a.insertPersonAlias('Dummy Person')
        #a.mapPersonAliasToRoleToSong(dummyPerson, 
